...
if __name__=='__main__':
    lysurl='http://liangyusheng.zuopinj.com/'
    booksurl='http://liangyusheng.zuopinj.com/'
    ...
    if os.path.exists('D:\\项目\\成语分级\\语料\\data\\titles.txt'):
        ...
        titles=set()
    books=parsebookhtml(booksurl,lysurl)
    #nbook=0
    for title in books:
        if title in titles:
            print("重复小说")
        ...
        titles.add(title)
        link=books[title]
        seclinks=parsecataloguehtml(link,lysurl)#某本书所有章节的链接
        articals=[]
        count=0
